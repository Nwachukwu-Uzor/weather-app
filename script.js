...
const fetchWeatherData = async (cityName) => {
    try {
        const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=0393134d3a91e2d6a6c38f1165ebd58f`)
        const data = await response.json()
        console.log(data)
        populateResultDiv(data) 
    } catch (error) {
        console.log(error)
    }
}

const populateResultDiv = result => {
    let data = ''
    if (result.cod === 200) {
        const responseCountry = countries.filter(country => country["code"] === result.sys.country)
        data = `
            <h3>Weather Information for ${result.name}</h3>
            <h4>Decription: <span class='description'>${result.weather[0].description}</span></h4>
            <h4>Temperature: <span class='temp'>${result.main.temp} &#176C</span></h4>
            <h4>Pressure: <span class='pressure'>${result.main.pressure}</span></h4>
            <h4>Windspeed: <span class='windspeed'>${result.wind.speed}m/s</span></h4>
            <h4>Country: ${responseCountry[0]["name"]}</h4>
        `
    } else {
        data = `
            <h4>Error: ${result.message}</h4>
        `
    }
